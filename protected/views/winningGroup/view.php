	...
	<?php $this->widget('zii.widgets.CDetailView',array(
		...
		'attributes'=>array(
			...
			'toto_result_id'=>array(
				...
				'value'=>TotoResult::model()->findByPk($model->toto_result_id)->date
			)
		),
	)); ?>
</div>
<div class="col-md-3">
	<div class="widget">
		...
		<ul class="list-group">
			<li class="list-group-item"><a href="<?= $b ?>/<?= get_class($model) ?>/update/<?= $model->id ?>">Update
					This <?= get_class($model) ?></a></li>
			<li class="list-group-item"><a href="<?= $b ?>/<?= get_class($model) ?>">List <?= get_class($model) ?></a>
			</li>
			<li class="list-group-item"><a
					href="<?= $b ?>/<?= get_class($model) ?>/create">Create <?= get_class($model) ?></a></li>
			<li class="list-group-item"><a
					href="<?= $b ?>/<?= get_class($model) ?>/admin">Manage <?= get_class($model) ?>s</a></li>
		</ul>
	</div>
</div>